    ...
    for (_matrix, matrix_sprite) in matrix_query.iter_mut() {
        for (position, mut transform) in block_query.iter_mut() {
            let new_x: f32 = ((position.x as f32 * Block::SIZE) - (matrix_sprite.size.x * 0.5))
                + (Block::SIZE * 0.5);
            let new_y: f32 = (matrix_sprite.size.y * -0.5)
                + (position.y as f32 * Block::SIZE)
                + (Block::SIZE * 0.5);
        ...
        commands
            .spawn(SpriteBundle {
                material: materials.add(
                    Color::rgb(block.0.color.r(), block.0.color.g(), block.0.color.b()).into(),
                ),
